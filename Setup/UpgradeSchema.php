...
namespace Mavenbird\Shopbybrand\Setup;
...
/**
 * Class UpgradeSchema
 * @package Mavenbird\Shopbybrand\Setup
 */
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '2.2.0', '<')) {
            if (!$installer->tableExists('mavenbird_shopbybrand_category')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('mavenbird_shopbybrand_category'))
                    ...
                    ->addIndex(
                        $installer->getIdxName('mavenbird_shopbybrand_category', 'url_key'),
                        'url_key',
                        ['type' => AdapterInterface::INDEX_TYPE_UNIQUE]
                    )
                    ->setComment('Mavenbird Shopbybrand category table');

                $installer->getConnection()->createTable($table);
            }

            if (!$installer->tableExists('mavenbird_shopbybrand_brand_category')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('mavenbird_shopbybrand_brand_category'))
                    ...
                    ->addIndex(
                        $installer->getIdxName('mavenbird_shopbybrand_brand_category', ['option_id']),
                        ['option_id']
                    )
                    ->addIndex($installer->getIdxName('mavenbird_shopbybrand_brand_category', ['cat_id']), ['cat_id'])
                    ->addForeignKey(
                        $installer->getFkName(
                            'mavenbird_shopbybrand_brand_category',
                            'option_id',
                    ...
                    ->addForeignKey(
                        $installer->getFkName(
                            'mavenbird_shopbybrand_brand_category',
                            'cat_id',
                            'mavenbird_shopbybrand_category',
                            'cat_id'
                        ),
                        'cat_id',
                        $installer->getTable('mavenbird_shopbybrand_category'),
                        'cat_id',
                        Table::ACTION_CASCADE
                    )
                    ->addIndex(
                    ...
                    ->setComment('Mavenbird Shopbybrand brand category table');
